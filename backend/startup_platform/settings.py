...
ALLOWED_HOSTS = config('ALLOWED_HOSTS', default='localhost,127.0.0.1,localhost:5173,127.0.0.1:5173,localhost:8000,127.0.0.1:8000', cast=lambda v: [s.strip() for s in v.split(',')])
...
# Custom Authentication Backends (for bcrypt password support in admin)
AUTHENTICATION_BACKENDS = [
    'api.backends.BcryptAuthBackend',  # Custom bcrypt backend
...
DEFAULT_FROM_EMAIL = config('EMAIL_HOST_USER', default='noreply@startupplatform.com')

# Server email (for error emails)
SERVER_EMAIL = config('EMAIL_HOST_USER', default='noreply@startupplatform.com')
